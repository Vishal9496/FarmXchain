...
import { analyzeImageWithAI, AI_SOURCE } from "../services/aiService";
...
const AIQualityCheck = () => {
  ...
  // ✅ P0-10: visible error state. A failed analysis must be SEEN, never silently
  ...
  const analyzeProduct = async () => {
    ...
    try {
      ...
      // analyzeImageWithAI now either resolves with a real, tagged result or
      // throws — there is no third outcome where a failure quietly becomes a
      // plausible-looking invented result.
      ...
      setResult({
        ...
        aiQuality,
        aiAnalysis,
        // ✅ P0-10: carried through so the results panel can render the badge.
        source: ai.source,
      });
    } catch (error) {
      // ✅ P0-10: the failure is shown to the user, not swallowed. No result is
      // set, so the UI cannot render a stale or fabricated card in its place.
      console.error("AI analysis failed:", error);
      setResult(null);
      setAnalysisError(
        error?.message ||
          "Quality check failed. Please try again in a moment.",
      );
    } finally {
      setAnalyzing(false);
    }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="backdrop-blur-xl bg-white/80 border border-white/50 rounded-3xl shadow-xl p-8">
            ...
            {/* ✅ P0-10: visible, styled error state — the direct replacement for the
                deleted silent mock fallback. No result card renders alongside this. */}
            {!analyzing && analysisError && (
              ...
              >
                ...
                <p className="text-gray-600 text-sm mb-6">{analysisError}</p>
                <button
                  onClick={analyzeProduct}
            ...
            {result && (
              <div className="space-y-6">
                {/* ✅ P0-10: source badge. Always rendered when a result exists, so a
                    simulated result can never be mistaken for a real measurement. */}
                {result.source === AI_SOURCE.MOCK ? (
                  <div className="flex items-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-4 py-2.5 text-sm font-medium text-amber-800">
                    ...
                    <span>
                      Simulated result — development mode only. Not a real AI
                      measurement.
                    </span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm font-medium text-emerald-800">
                    ...
                    <span>Verified by Gemini AI</span>
                  </div>
                )}
